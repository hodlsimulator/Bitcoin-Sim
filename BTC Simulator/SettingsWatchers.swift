...
struct UnifiedValueWatchersA: View {
    ...
    private var watchersA1: some View {
        EmptyView()
            .onChange(of: simSettings.halvingBumpUnified) { oldVal, newVal in
            ...
            .onChange(of: simSettings.maxDemandBoostUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
            ...
            .onChange(of: simSettings.maxCountryAdBoostUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
    ...
    private var watchersA2: some View {
        EmptyView()
            .onChange(of: simSettings.maxClarityBoostUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
            ...
            .onChange(of: simSettings.maxEtfBoostUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
            ...
            .onChange(of: simSettings.maxTechBoostUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
            ...
            .onChange(of: simSettings.maxScarcityBoostUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
...
struct UnifiedValueWatchersB: View {
    ...
    private var watchersB1a: some View {
        EmptyView()
            .onChange(of: simSettings.maxMacroBoostUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
            ...
            .onChange(of: simSettings.maxStablecoinBoostUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
    ...
    private var watchersB1b: some View {
        EmptyView()
            .onChange(of: simSettings.maxDemoBoostUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
            ...
            .onChange(of: simSettings.maxAltcoinBoostUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
    ...
    private var watchersB2: some View {
        EmptyView()
            .onChange(of: simSettings.adoptionBaseFactorUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
            ...
            .onChange(of: simSettings.maxClampDownUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
            ...
            .onChange(of: simSettings.maxCompetitorBoostUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
...
struct UnifiedValueWatchersC: View {
    ...
    private var watchersC1a: some View {
        EmptyView()
            .onChange(of: simSettings.breachImpactUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
            ...
            .onChange(of: simSettings.maxPopDropUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
    ...
    private var watchersC1b: some View {
        EmptyView()
            .onChange(of: simSettings.maxMeltdownDropUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
            ...
            .onChange(of: simSettings.blackSwanDropUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
    ...
    private var watchersC2: some View {
        EmptyView()
            .onChange(of: simSettings.bearWeeklyDriftUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
            ...
            .onChange(of: simSettings.maxMaturingDropUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
            ...
            .onChange(of: simSettings.maxRecessionDropUnified) { oldVal, newVal in
                guard simSettings.periodUnit == .weeks else { return }
                guard newVal != oldVal else { return }
