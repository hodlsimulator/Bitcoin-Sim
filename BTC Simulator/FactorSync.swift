...
extension SimulationSettings {
    ...
    func syncAllFactorsToIntensity(_ t: Double, simSettings: SimulationSettings) {
        // 1) If we are in an extreme-bearish or extreme-bullish lock, skip all factor syncing
        //    so that turned-off factors remain pinned at min/max and don't move.
        if chartExtremeBearish || chartExtremeBullish {
            return
        }
        ...
        if periodUnit == .weeks {
            ...
            // Halving
            if let frac = factorEnableFrac["Halving"], frac > 0.0, !simSettings.lockedFactors.contains("Halving") {
                syncOneFactor(
                    factorName: "Halving",
                    t: t,
            ...
            if let frac = factorEnableFrac["InstitutionalDemand"], frac > 0.0, !simSettings.lockedFactors.contains("InstitutionalDemand") {
                syncOneFactor(
            ...
            // Country Adoption
            if let frac = factorEnableFrac["CountryAdoption"], frac > 0.0, !simSettings.lockedFactors.contains("CountryAdoption") {
                syncOneFactor(
            ...
            // Regulatory Clarity
            if let frac = factorEnableFrac["RegulatoryClarity"], frac > 0.0, !simSettings.lockedFactors.contains("RegulatoryClarity") {
                syncOneFactor(
            ...
            // ETF Approval
            if let frac = factorEnableFrac["EtfApproval"], frac > 0.0, !simSettings.lockedFactors.contains("EtfApproval") {
                syncOneFactor(
            ...
            // Tech Breakthrough
            if let frac = factorEnableFrac["TechBreakthrough"], frac > 0.0, !simSettings.lockedFactors.contains("TechBreakthrough") {
                syncOneFactor(
            ...
            // Scarcity Events
            if let frac = factorEnableFrac["ScarcityEvents"], frac > 0.0, !simSettings.lockedFactors.contains("ScarcityEvents") {
                syncOneFactor(
            ...
            // Global Macro Hedge
            if let frac = factorEnableFrac["GlobalMacroHedge"], frac > 0.0, !simSettings.lockedFactors.contains("GlobalMacroHedge") {
                syncOneFactor(
            ...
            // Stablecoin Shift
            if let frac = factorEnableFrac["StablecoinShift"], frac > 0.0, !simSettings.lockedFactors.contains("StablecoinShift") {
                syncOneFactor(
            ...
            if let frac = factorEnableFrac["DemographicAdoption"], frac > 0.0, !simSettings.lockedFactors.contains("DemographicAdoption") {
                syncOneFactor(
            ...
            // Altcoin Flight
            if let frac = factorEnableFrac["AltcoinFlight"], frac > 0.0, !simSettings.lockedFactors.contains("AltcoinFlight") {
                syncOneFactor(
            ...
            // AdoptionFactor
            if let frac = factorEnableFrac["AdoptionFactor"], frac > 0.0, !simSettings.lockedFactors.contains("AdoptionFactor") {
                syncOneFactor(
            ...
            // RegClampdown
            if let frac = factorEnableFrac["RegClampdown"], frac > 0.0, !simSettings.lockedFactors.contains("RegClampdown") {
                syncOneFactor(
            ...
            // CompetitorCoin
            if let frac = factorEnableFrac["CompetitorCoin"], frac > 0.0, !simSettings.lockedFactors.contains("CompetitorCoin") {
                syncOneFactor(
            ...
            // SecurityBreach
            if let frac = factorEnableFrac["SecurityBreach"], frac > 0.0, !simSettings.lockedFactors.contains("SecurityBreach") {
                syncOneFactor(
            ...
            // BubblePop
            if let frac = factorEnableFrac["BubblePop"], frac > 0.0, !simSettings.lockedFactors.contains("BubblePop") {
                syncOneFactor(
            ...
            // StablecoinMeltdown
            if let frac = factorEnableFrac["StablecoinMeltdown"], frac > 0.0, !simSettings.lockedFactors.contains("StablecoinMeltdown") {
                syncOneFactor(
            ...
            // BlackSwan
            if let frac = factorEnableFrac["BlackSwan"], frac > 0.0, !simSettings.lockedFactors.contains("BlackSwan") {
                syncOneFactor(
            ...
            // BearMarket
            if let frac = factorEnableFrac["BearMarket"], frac > 0.0, !simSettings.lockedFactors.contains("BearMarket") {
                syncOneFactor(
            ...
            // MaturingMarket
            if let frac = factorEnableFrac["MaturingMarket"], frac > 0.0, !simSettings.lockedFactors.contains("MaturingMarket") {
                syncOneFactor(
            ...
            // Recession
            if let frac = factorEnableFrac["Recession"], frac > 0.0, !simSettings.lockedFactors.contains("Recession") {
                syncOneFactor(
